    ...
    details: [
      {
        icon: Layers,
        label: "Architecture",
        body: "Event-driven microservices with CQRS. Kafka-backed event store with PostgreSQL read replicas. Redis L1 cache delivers sub-5ms p95 on all hot paths.",
      },
      {
        icon: Activity,
        label: "Scalability",
        body: "Horizontal autoscaling via Kubernetes HPA. Stateless services share Redis session store. Resilience4j circuit breakers on every downstream call.",
      },
      {
        icon: Database,
        label: "Database",
        body: "Normalized PostgreSQL with promoted read replicas. Optimistic locking for concurrent transactions. Hash-partitioned audit log tables.",
      },
      {
        icon: Globe,
        label: "API Strategy",
        body: "REST/HTTPS with JWT auth. Gateway-level rate limiting (100 req/s per tenant). OpenAPI 3.0 spec with consumer-driven contract tests.",
      },
      {
        icon: Box,
        label: "Deployment",
        body: "Docker on AWS EKS. GitHub Actions CI/CD with blue-green deploys. Prometheus + Grafana observability. Full distributed tracing via Jaeger.",
      },
    ],
  },
  {
    ...
    metrics: [
      { l: "Auth Latency", v: "<2ms" },
      { l: "Token Store", v: "Redis" },
      { l: "Model", v: "Zero-trust" },
    ],
    details: [
      {
        icon: Shield,
        label: "Architecture",
        body: "Stateless JWT with refresh token rotation. Redis blacklist enables instant revocation without DB round-trips on the auth hot path.",
      },
      {
        icon: Activity,
        label: "Scalability",
        body: "Stateless design allows unlimited horizontal scaling. 98% of token validations resolved from Redis — zero DB hits for active sessions.",
      },
      {
        icon: Database,
        label: "Database",
        body: "PostgreSQL user store with bcrypt (cost=12). Redis blacklist with TTL-expiry. Indexes on email + OAuth provider for sub-ms lookups.",
      },
      {
        icon: Lock,
        label: "Security",
        body: "Refresh token rotation on every use. Per-device token tracking. Revocation broadcast via Redis pub/sub to all service instances in <50ms.",
      },
      {
        icon: Box,
        label: "Deployment",
        body: "Standalone Docker service injectable as a sidecar. AWS Secrets Manager for credential rotation. Zero-downtime key rotation supported.",
      },
    ],
  },
  {
